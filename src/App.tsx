...
if (!isTouchDevice) {
  ...
  if (cursorDot && cursorOutline) {
    ...
    let mouseX = 0, mouseY = 0, outlineX = 0, outlineY = 0, cursorVisible = false;
...
const timer = setTimeout(() => {
  ...
  // KEY FIX: no x/y animation on image — only opacity + scale + blur
  gsap.set('.image-anim', { opacity: 0, scale: 1.04, filter: 'blur(15px)' });
  gsap.set('.fade-in-elem', { opacity: 0, filter: 'blur(4px)' });

  const tlIntro = gsap.timeline();
  tlIntro
    .to('.nav-anim', { opacity: 1, y: 0, duration: 1.2, ease: 'power3.out', delay: 0.2 })
    .to(heroTitles.chars, { y: 0, opacity: 1, filter: 'blur(0px)', duration: 1.5, stagger: 0.02, ease: 'power4.out' }, '-=0.8')
    .to('.image-anim', { opacity: 1, scale: 1, filter: 'blur(0px)', duration: 1.8, ease: 'expo.out' }, '-=1.2')
    .to('.fade-in-elem', { opacity: 1, filter: 'blur(0px)', duration: 1, stagger: 0.1, ease: 'power2.out' }, '-=1.0');

  gsap.fromTo('.parallax-text',
    { yPercent: 0, opacity: 1, filter: 'blur(0px)', scale: 1 },
    {
      scrollTrigger: { trigger: '#hero', start: 'top top', end: 'bottom top', scrub: true },
      yPercent: 50, opacity: 0, filter: 'blur(15px)', scale: 0.9, ease: 'none',
    }
  );
  gsap.fromTo('.parallax-img',
    ...
    {
      scrollTrigger: { trigger: '#hero', start: 'top top', end: 'bottom top', scrub: true },
      yPercent: 20, scale: 0.95, filter: 'blur(8px) grayscale(50%)', ease: 'none',
    }
  );
  ...
  gsap.fromTo('.footer-content',
    { y: 50, opacity: 0, filter: 'blur(10px)' },
    { y: 0, opacity: 1, filter: 'blur(0px)', duration: 1.2, ease: 'power3.out',
      scrollTrigger: { trigger: '#footer-trigger', start: 'top 70%', toggleActions: 'play none none reverse' } }
  );
}, 100);
...
```
  ...
  <nav className="w-full flex items-center justify-between px-6 md:px-12 py-6 fixed top-0 z-50 mix-blend-difference opacity-0 nav-anim">
    ...
    <div className="hidden md:flex items-center space-x-10 text-sm font-medium text-gray-300">
      <a href="#vision" className="hoverable hover:text-white transition-colors relative group">
        Vision
        <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-brandRed transition-all duration-300 group-hover:w-full"></span>
      </a>
      <a href="#ventures" className="hoverable hover:text-white transition-colors relative group">
        Ventures
        <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-brandRed transition-all duration-300 group-hover:w-full"></span>
      </a>
    </div>
    <div className="magnetic-wrap hidden md:block">
      <a href="#contact" className="magnetic-btn hoverable bg-white text-black px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-brandRed hover:text-white transition-colors duration-300 inline-block">
        Let's Talk
      </a>
    </div>
  </nav>
  ...
  <div id="smooth-wrapper" className="relative z-10 bg-[#050505] mb-[80vh]">
    <div id="smooth-content">
      ...
      {/* ============================================================
          ...
          Use "inset-x-0" (left:0; right:0) for ALL absolutely positioned
          elements. This means width = 100% of parent, so centering with
          text-center or flexbox is always reliable.
          NEVER use left-1/2 + -translate-x-1/2 for wide elements —
          this breaks when the element is wider than viewport on mobile.
      ============================================================ */}
      <section id="hero" className="relative w-full h-[100dvh] overflow-hidden">
        <div className="ambient-glow-red"></div>

        {/* Top intro */}
        <div className="absolute inset-x-0 top-[12%] md:top-[18%] z-30 opacity-0 fade-in-elem text-center px-4">
          <p className="text-gray-400 font-medium text-sm md:text-lg tracking-wide">
            Hi, I'm Alif Shahariar. Founder of{' '}
            <a href="#" className="text-white hover:text-brandRed transition-colors hoverable border-b border-gray-600 hover:border-brandRed pb-0.5">
              Examio.xyz
            </a>
          </p>
        </div>
        ...
        {/* 
          TEXT LAYER BEHIND IMAGE (z-10)
          Only "TECH FOUNDER" is actually visible here.
          "& AI ENTHUSIAST" is invisible (opacity-0) just to hold spacing.
        */}
        <div
          className="parallax-text pointer-events-none absolute inset-x-0 flex flex-col items-center"
        ...
        >
          ...
          <div className="overflow-hidden w-full text-center px-2 opacity-0 select-none" style={{ marginTop: '-1.5vw' }}>
        ...
        {/* 
          IMAGE LAYER (z-20)
          inset-x-0 + flex justify-center = always perfectly centered.
          No translate tricks. Width controlled via max-width on the img itself.
        */}
        <div
          className="parallax-img image-anim pointer-events-none absolute inset-x-0 bottom-0 flex justify-center items-end"
          style={{ top: '14%', zIndex: 20 }}
        >
          <img
            src="https://res.cloudinary.com/dejm7pz1d/image/upload/v1771718749/IMG_3072_m8dqgg.png"
            alt="Alif Shahariar"
            className="founder-image-glow pointer-events-auto block"
            style={{
        ...
        {/* 
          TEXT LAYER IN FRONT OF IMAGE (z-30)
          Only "& AI ENTHUSIAST" is visible here.
          "TECH FOUNDER" is invisible just to hold spacing.
        */}
        <div
          className="parallax-text pointer-events-none absolute inset-x-0 flex flex-col items-center"
          style={{ top: '28%', zIndex: 30 }}
        >
          <div className="overflow-hidden w-full text-center px-2 opacity-0 select-none">
            <h1 className="hero-title font-display uppercase tracking-tight m-0 leading-none">
              Tech Founder
            </h1>
          </div>
          <div className="overflow-hidden w-full text-center px-2 pointer-events-auto hoverable" style={{ marginTop: '-1.5vw' }}>
        ...
        <div className="absolute bottom-6 left-6 md:bottom-8 md:left-12 z-30 opacity-0 fade-in-elem">
          <div className="text-xs md:text-sm text-gray-500">
            based in<br />
            <span className="text-gray-300 font-medium">Natore, Bangladesh.</span>
          </div>
        </div>

        {/* Bottom right: scroll — desktop only */}
        <div className="absolute bottom-12 right-6 md:right-12 z-30 opacity-0 fade-in-elem hidden md:flex flex-col items-center gap-2">
          <span className="text-[10px] uppercase tracking-[0.3em] text-gray-500 rotate-90 translate-y-6">Scroll</span>
      ...
      <div className="bg-brandRed text-white py-4 border-y border-red-900 rotate-[-2deg] scale-105 my-10 md:my-20 overflow-hidden relative z-20 shadow-2xl">
        <div className="marquee-wrapper font-display text-4xl md:text-5xl uppercase tracking-wider">
          <div className="flex whitespace-nowrap gap-8 px-4 items-center">
            <span>Founder @ Examio.xyz</span> <i className="ph-fill ph-star-four text-2xl"></i>
            <span>AI Innovations</span> <i className="ph-fill ph-star-four text-2xl"></i>
            <span>Tech Entrepreneur</span> <i className="ph-fill ph-star-four text-2xl"></i>
          </div>
          <div className="flex whitespace-nowrap gap-8 px-4 items-center" aria-hidden="true">
            <span>Founder @ Examio.xyz</span> <i className="ph-fill ph-star-four text-2xl"></i>
            <span>AI Innovations</span> <i className="ph-fill ph-star-four text-2xl"></i>
            <span>Tech Entrepreneur</span> <i className="ph-fill ph-star-four text-2xl"></i>
          </div>
      ...
      {/* Vision Section */}
      <section id="vision" className="min-h-[80vh] py-24 md:py-32 px-5 md:px-12 w-full flex flex-col justify-center items-center relative z-10">
          ...
          <div className="mt-20 grid grid-cols-1 md:grid-cols-2 gap-12 text-left w-full pt-16 relative">
            ...
            <div className="reveal-on-scroll">
              ...
              <p className="text-gray-400 text-base leading-relaxed">
                Leading the vision, strategy, and execution of next-generation digital platforms designed to create massive impact in the modern ecosystem.
              </p>
            </div>
            <div className="reveal-on-scroll">
              <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-gray-400 text-base leading-relaxed">
                Deeply exploring how Artificial Intelligence and Machine Learning technologies can be leveraged to completely disrupt traditional industries.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Ventures Section */}
      <section id="ventures" className="min-h-screen py-24 md:py-32 px-5 md:px-12 w-full flex items-center relative z-10 bg-[#050505]">
        <div className="w-full h-[1px] bg-gray-900 absolute top-0 left-0 line-reveal"></div>
          ...
          <div className="flex flex-col w-full relative">
            ...
            <div className="reveal-on-scroll py-12 md:py-20 flex flex-col md:flex-row md:items-center justify-between group cursor-pointer hoverable relative">
              ...
              <div className="md:w-1/2">
                <h3 className="text-5xl md:text-7xl lg:text-[7rem] font-display uppercase text-white group-hover:text-transparent transition-all duration-500 mb-6 md:mb-0 leading-none break-words" style={{ WebkitTextStroke: '0px' }}>
                  Examio.xyz
                </h3>
              </div>
              <div className="md:w-1/2 md:pl-12 flex flex-col items-start">
                <p className="text-gray-400 text-base md:text-lg mb-8 max-w-md">
                  A next-generation digital platform scaling education, assessment, and tech infrastructure through AI-driven methodologies.
                </p>
            ...
            <div className="reveal-on-scroll py-12 md:py-20 flex flex-col md:flex-row md:items-center justify-between group cursor-pointer hoverable relative">
              <div className="w-full h-[1px] bg-gray-800 absolute bottom-0 left-0 line-reveal"></div>
              <div className="md:w-1/2">
                <h3 className="text-5xl md:text-7xl lg:text-[7rem] font-display uppercase text-white group-hover:text-transparent transition-all duration-500 mb-6 md:mb-0 leading-none break-words">
                  AI Labs
                </h3>
              </div>
              <div className="md:w-1/2 md:pl-12 flex flex-col items-start">
                <p className="text-gray-400 text-base md:text-lg mb-8 max-w-md">
                  Internal research initiatives focusing on Machine Learning workflow optimization and automated intelligent agents.
                </p>
                <div className="flex items-center gap-4">
                  <p className="text-brandRed text-[10px] md:text-xs tracking-[0.2em] uppercase font-bold border border-gray-800 px-4 py-2 rounded-full group-hover:border-brandRed transition-colors">
                    ML • Automation
      ...
      <div id="footer-trigger" className="h-[80vh] w-full bg-transparent pointer-events-none"></div>
    </div>
  </div>

  {/* Fixed Footer */}
  <footer id="contact" className="fixed bottom-0 left-0 w-full h-[80vh] flex flex-col items-center justify-center bg-[#050505] z-0 px-5 md:px-12 border-t border-gray-900">
    <div className="footer-content max-w-7xl mx-auto flex flex-col items-center text-center w-full relative z-10">
      <p className="text-brandRed uppercase tracking-[0.3em] text-[10px] md:text-xs font-bold mb-6">Ready to innovate?</p>
      ...
      <div className="magnetic-wrap mt-2 md:mt-4">
        <a href="mailto:alif@examio.xyz" className="magnetic-btn inline-block text-xl sm:text-2xl md:text-4xl font-medium border-b-2 border-brandRed pb-1 md:pb-2 hover:text-brandRed transition-colors hoverable break-all">
          alif@examio.xyz
        </a>
      </div>
      <div className="mt-16 md:mt-24 flex flex-wrap justify-center gap-6 md:gap-12 text-gray-500 text-sm md:text-lg uppercase tracking-wider font-semibold">
        <a href="#" className="hover:text-white transition-colors hoverable">Twitter</a>
        <a href="#" className="hover:text-white transition-colors hoverable">LinkedIn</a>
        <a href="#" className="hover:text-white transition-colors hoverable">Examio.xyz</a>
      </div>
      <p className="mt-12 md:mt-16 text-xs text-gray-700">© 2026 ALIF SHAHARIAR. BUILT IN BANGLADESH.</p>
    </div>
